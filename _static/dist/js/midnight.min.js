...
(function ($) {
	...
	$.widget("aerolab.midnight", {
		...
		_getContainerHeight: function () {
			var $customHeaders = this.element.find(
		...
		_setupHeaders: function () {
			...
			for (var headerClass in this._headers) {
				...
				if (typeof this._headers[headerClass].element === "undefined") {
					...
					if ($existingHeader.length) {
						...
						this._headers[headerClass].element = $defaultHeader
							.clone(true, true)
					...
					if (this._transformMode !== false) {
						this._headers[headerClass].element.css(
							this._transformMode,
							"translateZ(0)"
						);
					}
					...
					if (
						!this._headers[headerClass].element.find(
			...
			$customHeaders.each(function () {
				...
				if (!$header.find("> ." + context.options["innerClass"]).length) {
					$header.wrapInner(
						'<div class="' + context.options["innerClass"] + '"></div>'
					);
				}
		...
		_recalculateHeaders: function () {
			...
			if (typeof window.getComputedStyle === "function") {
				...
				if (
					...
					if (
						transformArray !== null &&
						transformArray.length >= 6 &&
						!isNaN(parseFloat(transformArray[5]))
					) {
				...
				if (style.top.indexOf("%") >= 0 && !isNaN(parseFloat(styleTop))) {
					...
					styleTop.indexOf("px") >= 0 &&
					!isNaN(parseFloat(styleTop))
				) {
			...
			for (var ix = 0; ix < this._sections.length; ix++) {
				...
				if (
					headerEnd >= this._sections[ix].start &&
					headerStart <= this._sections[ix].end
				) {
					...
					if (
						headerStart >= this._sections[ix].start &&
						headerEnd <= this._sections[ix].end
					) {
					...
					// If the header is in the middle of the end of a section, it comes from the top
						...
						this._headers[this._sections[ix].className].from = "top";
						this._headers[this._sections[ix].className].progress =
							1.0 - (headerEnd - this._sections[ix].end) / headerHeight;
					}
					// If the header is in the middle of the start of a section, it comes from the bottom
						...
						if (this._headers[this._sections[ix].className].from === "top") {
							this._headers[this._sections[ix].className].progress +=
								(headerEnd - this._sections[ix].start) / headerHeight;
						} else {
		...
		_updateHeaders: function () {
			...
			if (totalProgress < 1.0) {
				...
				if (this._headers[this.options["defaultClass"]].from === "") {
					this._headers[this.options["defaultClass"]].from =
						this._headers[lastActiveClass].from === "top" ? "bottom" : "top";
					this._headers[this.options["defaultClass"]].progress =
						1.0 - totalProgress;
				} else {
					this._headers[this.options["defaultClass"]].progress +=
						1.0 - totalProgress;
			...
			for (var ix in this._headers) {
				...
				if (this._headers[ix].from === "top") {
					if (this._transformMode !== false) {
						this._headers[ix].element[0].style[this._transformMode] =
							"translateY(-" + offset + "%) translateZ(0)";
						this._headers[ix].inner[0].style[this._transformMode] =
							"translateY(+" + offset + "%) translateZ(0)";
					} else {
